...
for i in ${StringArray[@]}; do
   echo $i
   output_directory=$path/spurious_correlations_in_argmin/argument_mining_output/ArgumentQuality_UKP_MTL/"$i"/$seed
   echo "training..."
   python3 $path/mt-dnn/train.py --multi_gpu_on --init_checkpoint bert-base-uncased --data_dir $path/spurious_correlations_in_argmin/argument_mining_data/UKPArgMin/"$i"/bert_base_uncased_lower --train_datasets UKP,ArgumentQuality --test_datasets UKP,UKP_heldout,ArgumentQuality --task_def $path/spurious_correlations_in_argmin/argument_mining_scripts/UKP_ArgumentQuality_mtl_taskdef.yml --epochs 10 --seed $seed --batch_size 5 --batch_size_eval 5 --output_dir $output_directory
