...
for i in ${StringArray[@]}; do
   echo $i
   output_directory=$path/spurious_correlations_in_argmin/argument_mining_output/VacClaim_UKP_MT/"$i"/$seed
   echo "training..."
   python3 $path/mt-dnn/train.py --multi_gpu_on --tensorboard --epochs 10 --init_checkpoint bert-base-uncased --data_dir $path/spurious_correlations_in_argmin/argument_mining_data/UKPArgMin/"$i"/bert_base_uncased_lower --train_datasets UKP,VaccinationCorpusClaims --test_datasets UKP,UKP_heldout,VaccinationCorpusClaims --task_def $path/spurious_correlations_in_argmin/argument_mining_scripts/taskdefs/all_tasks_taskdef.yml --seed $seed --batch_size 5 --batch_size_eval 5 --output_dir $output_directory
